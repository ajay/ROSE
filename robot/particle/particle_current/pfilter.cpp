...
void pfilter::move(vec sensors) {
	...
	double t2iK_rotation = 0.039893206349 * 180 / M_PI / 2;
	dist(0) *= -1;
	dist(2) *= -1;
...
void pfilter::weigh(mat &observations) {
    ...
    vec observation = observations.col(i).subvec(0,1);
		R[i] = eucdist(observation);
		T[i] = angle(observation);
	}
	for (int i = 0; i < particles.size(); i++){
		for(int j = 0; j < landmarks.size(); j++){
			...
			if (observations(2, j) > 0.5) {
				vec diff({ landmarks[j].x - particles[j].x, landmarks[j].y - particles[j].y });
				radius[j] = eucdist(diff);
				theta[j] = angle(diff) - particles[i].t; // theta of the robot
